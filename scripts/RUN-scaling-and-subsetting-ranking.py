...
for scaling in scalings_list:
    ...
    symlog_PC_sources = preprocess.log_transform(PC_sources,
    ...
    np.savetxt('../results/REPRODUCE-RESULTS-' + data_tag + '-perc-of-data-' + str(sample_percentage) + '-' + str(n_components) + 'D-LDM-optimizing-on-' + '-'.join(depvar_names) + '-' + scaling + '-scaling-cost.csv', ([cost]), delimiter=',', fmt='%.16e')

# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Run ranking of scaling with variable selection
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
for scaling in scalings_list:
    ...
    np.savetxt('../results/REPRODUCE-RESULTS-' + data_tag + '-perc-of-data-' + str(sample_percentage) + '-' + str(n_components) + 'D-LDM-optimizing-on-' + '-'.join(depvar_names) + '-' + scaling + '-scaling-backward-variable-elimination-selected-variables.csv', (selected_variables), delimiter=',', fmt='%.16e')

    np.savetxt('../results/REPRODUCE-RESULTS-' + data_tag + '-perc-of-data-' + str(sample_percentage) + '-' + str(n_components) + 'D-LDM-optimizing-on-' + '-'.join(depvar_names) + '-' + scaling + '-scaling-backward-variable-elimination-cost.csv', ([optimized_cost]), delimiter=',', fmt='%.16e')
